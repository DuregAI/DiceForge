...
internal sealed class MenuCharacterMotion
{
    ...
    public void Draw(MeshGenerationContext context, float width, float height, float time)
    {
        ...
        if (flag != null)
            Patch(context, flag, new Rect(1485, 130, 148, 114), new Rect(106f / 1536, 69f / 1024, 1357f / 1536, 885f / 1024), offset, scale, settings.flagEnabled ? time : 0, true, 1);
        ...
        Vector2 offset, float scale, float time, bool waving, float opacity)
    {
        ...
        for (int x = 0; x <= columns; x++)
        {
            ...
            float wave = waving ? Mathf.Sin(u * Mathf.PI) * Mathf.Sin(u * 9 - time * 2.8f) * 8f * (0.7f + 0.3f * Mathf.Sin(time * 0.6f)) : 0;
            float edge = waving ? 1 : Mathf.Clamp01(Mathf.Min(Mathf.Min(u, 1 - u), Mathf.Min(v, 1 - v)) * 8);
            mesh.SetNextVertex(new Vertex
            {
                position = new Vector3(offset.x + (rect.x + u * rect.width) * scale, offset.y + (rect.y + v * rect.height + wave - (waving ? u * 29 : 0)) * scale, Vertex.nearZ),
